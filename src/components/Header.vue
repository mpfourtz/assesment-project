<template>
    <div :class="{ 'sticky-header': isSticky }" class=" top-0 left-0 w-full shadow-md transition-all head-back">
      <div class="container mx-auto flex justify-between items-center p-4 m-auto prc-90">
        <!-- Logo -->
        <router-link to="/" class="text-2xl font-bold">MyLogo</router-link>
        ...
        <nav class="hidden md:flex space-x-6 white-color uppercase">
          <div class="relative group">
            ...
            <div class="absolute left-0 mt-2 w-48 shadow-lg opacity-0 group-hover:opacity-100 transition head-back">
              <router-link to="/category/action" class="block px-4 py-2">Action</router-link>
              <router-link to="/category/comedy" class="block px-4 py-2">Comedy</router-link>
            </div>
          </div>
